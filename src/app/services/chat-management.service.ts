...
export class ChatManagementService {
	...
	async sendChatMessage(content: string, currentUser: UserData, channelId: string): Promise<void> {
		const text = content.trim();
		if (!channelId || !text) {
			console.warn('sendChatMessage: Abbruch – fehlende channelId oder leerer Inhalt.', {
				channelId,
		...
		try {
			console.log('Sende Nachricht nach:', `channels/${channelId}/messages`, message);
			await this.chatService.sendMessage(channelId, message);
			console.log('Senden OK');
		} catch (error) {
