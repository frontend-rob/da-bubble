...
export class MainMenuComponent implements OnInit, OnDestroy {
			...
			let dmChannel: ChannelData | undefined =
				this.directMessageChannels.find((channel) =>
					channel.channelMembers.some(
						(member) => member?.uid === clickedUser.uid
					)
				);
		...
		combineLatest([
			...
			.subscribe(async ([channels, users]) => {
				...
				regularChannels.forEach((ch, index) => {
					console.log(
						`${index + 1}. ${ch.channelName} (ID: ${ch.channelId})`
					);
				});
				...
				dmChannels.forEach((ch, index) => {
					const members =
						ch.channelMembers
							?.map((m) => m?.userName || "undefined")
							.join(" ↔ ") || "keine Members";
					console.log(
						`${index + 1}. ${ch.channelName} (ID: ${
							ch.channelId
						}) - Members: ${members}`
					);
				});
				...
				dmChannels.forEach((ch) => {
					if (ch.channelMembers && ch.channelMembers.length === 2) {
						...
						if (member1?.uid && member2?.uid) {
							const pairKey = [member1.uid, member2.uid]
								.sort()
								.join("|");
				...
				dmPairs.forEach((channels, pairKey) => {
					if (channels.length > 1) {
						console.log(
							`🔴 DUPLIKAT gefunden für Paar ${pairKey}:`
						);
						channels.forEach((ch, index) => {
							console.log(
								`  ${index + 1}. ${ch.channelName} (ID: ${
									ch.channelId
								}) - Erstellt: ${ch.createdAt}`
							);
						});
					}
				});
	...
	private async updateAvailableUsers() {
		...
		console.log(
			"DM Channels vor Update:",
			this.directMessageChannels.length
		);
		...
		console.log(
			"DM Channels nach Update:",
			this.directMessageChannels.length
		);
		console.log(
			"Verfügbare Users für neue DMs:",
			this.availableUsersForDM.length
		);
	...
	private filterValidChannels(channels: ChannelData[]): ChannelData[] {
		return channels.filter((channel) => {
			...
			if (
				...
				!member1.userName ||
				!member2.userName ||
				member1.userName === "undefined" ||
				member2.userName === "undefined"
			) {
				console.warn(
					"Defekter Channel gefiltert (undefined Members):",
					channel.channelId
				);
				return false;
			}
	...
	private removeDuplicateChannels(channels: ChannelData[]): ChannelData[] {
		...
		for (const channel of channels) {
			...
			if (!seenPairs.has(pairKey)) {
				...
				console.log(
					"✅ Einzigartiger DM-Channel behalten:",
					channel.channelName,
					pairKey
				);
			} else {
	...
	private getAvailableUsersForNewDM(): UserData[] {
		return this.allUsers.filter((user) => {
			...
			const existingDM = this.directMessageChannels.find((dmChannel) => {
				return dmChannel.channelMembers.some(
